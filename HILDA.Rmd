...
```{r analyze_annual_salary_complete, eval=T, include=T, echo=T, comment=NA, cache=TRUE}
library(mgcv)
...
model27log=(log(wsfei) ~      s(wave) +  s(hgage) + sex + ancob3 + edhigh1 + jbmo62 + edcoq + edqenr + fmfo61 + fmmo61 + esdtl + mrcurr + hhstate + jbmwpsz + jbocct + aneab)
fit27log = gam(model27log, data=x)
anova(fit27log)
plot(fit27log,pages=1)
...
```{r consider_age, eval=T, include=T, echo=F, comment=NA, cache=TRUE}
x$dob <- 2012 - x$hgage
