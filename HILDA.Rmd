...
The model fitted with the wager_perc variable (AIC=`r extractAIC(fitp)`) is a better choice than that with the annula salary wsfei (AIC=`r extractAIC(fit)`).
...
AIC improved: `r extractAIC(fitp2)`

```{r satisfaction_job_pay_perc_interaction_rnd, eval=T, include=T, echo=F, comment=NA, cache=F}
library(ordinal)
fitp2rnd <- clm(jbmspay ~ hgsex + hgage +  hhtype+ ancob3 + jbocct:ancob3 + wages_perc:ancob3 + (1|xwaveid), data=xsub)
summary(fitp2rnd)
extractAIC(fitp2rnd)
```
