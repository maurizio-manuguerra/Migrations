...
pdf(paste("analyses_2015.07.17.",count,".pdf"))
...
model27=(log(wsfei) ~ s(wave) +  hgage + I(hgage^2) + sex + ancob3 + edhigh1 + jbmo62 + edcoq3 + edqenr + fmfo61 + fmmo61 + esdtl + mrcurr + hhstate + jbmwpsz + jbocct  + other_household)
...
model27rnd=(log(wsfei) ~ s(wave) +  hgage + I(hgage^2) + sex + ancob3 + edhigh1 + jbmo62 + edcoq3 + edqenr + fmfo61 + fmmo61 + esdtl + mrcurr + hhstate + jbmwpsz + jbocct)
##
#n=10000 #sample size
#ids_sample=sample(ids, size=n, replace=F)
#isample=which(x$xwaveid %in% ids_sample)
#xsample=x[isample,]
##
#fit27rnd = gamm(model27rnd, random=list(xwaveid=~1), data=xsample)
fit27rnd = gamm(model27rnd, random=list(xwaveid=~1), data=x)
anova(fit27rnd$gam)
