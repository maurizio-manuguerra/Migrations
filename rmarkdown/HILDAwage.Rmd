...
model27=(log(wsfei) ~ s(wave) +  hgage + I(hgage^2) + sex + ancob3 + edhigh1 + jbmo62 + edcoq + edqenr + fmfo61 + fmmo61 + esdtl + mrcurr + hhstate + jbmwpsz + jbocct + aneab + other_household + esempst)
...
model27byLabourForceStatus=(log(wsfei) ~ s(wave_no_resp) + s(wave_FT) + s(wave_PT) + s(wave_lookingFT) + s(wave_lookingPT) + s(wave_not_employed_marg_attach) + s(wave_not_employed_not_marg_attach) + hgage + I(hgage^2) + sex + ancob3 + edhigh1 + other_household) 
...
model27byEmploymentStatus=(log(wsfei) ~ s(wave_empl1) + s(wave_empl2) + s(wave_empl3) + s(wave_empl4) + s(wave_empl5) + hgage + I(hgage^2) + sex + ancob3 + edhigh1 + other_household) 
...
```{r fit28byEducation, eval=F, include=T, echo=T, comment=NA, cache=F}
